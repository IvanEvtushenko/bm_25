...
Используется и для документов (при индексации), и для запросов (при поиске) —
один и тот же экземпляр, чтобы леммы совпадали.
"""
...
class Tokenizer:
    ...
    morph     — pymorphy3.MorphAnalyzer; держим один экземпляр на процесс
                (загрузка словаря ≈ 30 МБ).
    stopwords — копия глобального списка, можно расширить под домен.
    min_len   — отрезаем токены короче (отсекает однобуквенный мусор).
    _cache    — {raw_token: lemma}; pymorphy не быстрый, повторы дают разы.
    """
    morph: pymorphy3.MorphAnalyzer = field(
        default_factory=lambda: pymorphy3.MorphAnalyzer(path=pymorphy3_dicts_ru.get_path())
    )
